...
namespace footytips
{
    ...
    internal class Program
    {
        ...
        static void Main (string[] args)
        {
            ...
            var roundData = GetData (round).ToList ();

            var currentRound = roundData.Where (x => x.Round == round).ToList ();

            Console.WriteLine ("Current Count: " + currentRound.Count);
            var learning = new MachineLearning ();
            var training = learning.TrainData (roundData.Where (x => x.Round < round - 1));

            learning.Evaluate2 (training, roundData.Where (x => x.Round < round));
            ...
            {
                Console.WriteLine (
                    $"HomeTeam :{item.sentiment.HomeTeam} ({item.sentiment.HomeTeamLastWeekScore}) vs Away Team {item.sentiment.AwayTeam} ({item.sentiment.AwayTeamLastWeekScore})| Prediction: {item.prediction.Winner} - {item.prediction.Score[0]} {item.prediction.Score[1]}");
            }

            Console.WriteLine ();
        }

        private static List<NrlResult> ConvertToOtherModel (IEnumerable<NrlResult2> roundData)
        {
            return roundData.Select (x => new NrlResult
            {
                AwayTeam = x.AwayTeam,
                    HomeTeam = x.HomeTeam,
        ...
        private static IEnumerable<NrlResult> GetData (int currentRound)
        {
            var result = new List<NrlResult2> ();
            for (var i = 1; i < currentRound; i++)
            {
                CreateFlatModelForRound (i, result, true);
            }

            CreateFlatModelForRound (currentRound, result, false);
            return ConvertToOtherModel (result);
        }

        private static void CreateFlatModelForRound (int i, List<NrlResult2> result, bool includePre)
        {
            var data = JsonConvert.DeserializeObject<CombinedData> (
                File.ReadAllText (GetNrldataJsonFileName (i)));

            Func<NrlMatches, bool> predicate = x => x.MatchMode != "Pre";
            if (!includePre)
            {
                predicate = x => true;
            }
            ...
            result.AddRange (from game in data.CurrentRound.Where (predicate) let homeTeamLadder = ladderList.GetLadder (game.HomeTeam.NickName)
                ...
                select new NrlResult2
                {
                    Round = round,
                        ...
                        Label = game.HomeTeam.Score > game.AwayTeam.Score ? 0 : 1,
        ...
        private static IEnumerable<ClubLadder> GetLadderList (IWebDriver driver, int round)
        {
            ...
            return (from row in ladder.FindElements (By.ClassName ("ladder__row")) let club = row.FindElement (By.ClassName ("ladder__item--club-header"))
